data = [
    ...
    #singles incomplete sentences
    ("make file.cpp", [["file.cpp", "file"]]),
    ("make images", [["images", "folder"]]),  
    ("delete old", [["old", "folder"]]), 
    ("remove backup", [["backup", "folder"]]), 
    ("add backup.txt", [["backup.txt", "file"]]),  
    ("add draft.doc", [["draft.doc", "file"]]),
    ("delete build", [["build", "folder"]]),  
    ("create data", [["data", "folder"]]),
    ("erase logs", [["logs", "folder"]]),
    ("add results.csv", [["results.csv", "file"]]),
    ("remove script.py", [["script.py", "file"]]), 
    ("delete script.cpp", [["script.cpp", "file"]]), 
    ("create archive", [["archive", "folder"]]),
    ("make new test", [["test", "folder"]]),
    ("delete test.py", [["test.py", "file"]]),
    ("remove cache", [["cache", "folder"]]),
    ("create the config", [["config", "folder"]]),
    ("delete code", [["code", "folder"]]),
    ("make test_results", [["test_results", "folder"]]),
    ("add run.py", [["run.py", "file"]]),
    ("create setup", [["setup", "folder"]]),  
    ("remove output", [["output", "folder"]]),
    ("delete .bashrc", [[".bashrc", "file"]]),
    ("remove readme", [["readme", "file"]]),  
    ("make temp", [["temp", "folder"]]),
    ("add helper", [["helper", "file"]]),  
    ("delete index", [["index", "file"]]),

    #multiple folder or files
    ("Create folders named happy and jump.", [["happy", "folder"], ["jump", "folder"]]),
    ("Create folders named happy, jump.", [["happy", "folder"], ["jump", "folder"]]),
    ("Create two folders happy jump.", [["happy", "folder"], ["jump", "folder"]]),
    ...
    ("form new tmp1 and tmp_2 files", [["tmp1", "files"], ["tmp_2", "files"]]),
    ...
    ("form files named home.html loading.html game.html and home.css loading.css game.css",[["home.html","file"], ["loading.html","file"], ["game.html","file"], ["home.css","file"], ["loading.css","file"], ["game.css","file"]]),
    ("make these assignment files assgn1.py assgn2.py assgn3.py and assgn4.py",[["assgn1.py","file"], ["assgn2.py","file"], ["assgn3.py","file"], ["assgn4.py","file"]]),
    ("new nodes named publisher.cpp and subscriber.cpp",[["publisher.cpp","file"], ["subscriber.cpp","file"]]),
    ("add files happy.txt sad.txt angry.txt emotion.py",[["happy.txt","file"], ["sad.txt","file"], ["angry.txt","file"], ["emotion.py","file"]]),
    ("need these ai_terminal.py training_data.py",[["ai_terminal.py","file"], ["training_data.py","file"]]),
    ...
    ("delete folder named create_jobs", [["create_jobs", "folder"]]),
    ("form creative_songs folder",[["creative_songs","folder"]]),
    ("I request you to add these files rythm.txt rhyme.txt and song.c",[["rythm.txt","file"],["rhyme.txt","file"],["song.c","file"]]),
    ("create folder named creative_arts", [["creative_arts", "folder"]]),
    ("erase file named elimination-rounds", [["elimination-rounds", "file"]]),
    ("discard the folder removed_items", [["removed_items", "folder"]]),
    ("delete the folder named new_things", [["new_things", "folder"]]),
    ("wipe deleted_files.txt from system", [["deleted_files.txt", "file"]]),
    ("make deleted_folder directory again", [["deleted_folder", "folder"]]),
    ("form a folder called delete_this_later", [["delete_this_later", "folder"]]),
    ("delete_deleted from memory", [["delete_deleted", "folder"]]),
    ("make a new folder removed_data", [["removed_data", "folder"]]),
    ("erase everything inside delete_zone", [["delete_zone", "folder"]]),
    ("add a folder called to_be_deleted", [["to_be_deleted", "folder"]]),
    ("remove file obsolete_delete.json", [["obsolete_delete.json", "file"]]),
    ("form new folder named not_to_be_deleted", [["not_to_be_deleted", "folder"]]),
    ("wipe outdated_delete_folder completely", [["outdated_delete_folder", "folder"]]),
    ("delete the folder deletion_test", [["deletion_test", "folder"]]),
    
    #confusing multiple folders andd files
    ("create folders final_delete and safe_zone", [["final_delete", "folder"], ["safe_zone", "folder"]]),
    ("add test_file.txt and delete_this.txt", [["test_file.txt", "file"], ["delete_this.txt", "file"]]),
    ("build remove_later maybe_delete_these folder", [["remove_later", "folder"],["maybe_delete_these", "folder"]]),
    ("make keep_folder and discard_folder", [["keep_folder", "folder"], ["discard_folder", "folder"]]),
    ("erase archive_delete_2021, archive_delete_2022, archive_delete_2023 and archive_delete_2024", [["archive_delete_2021", "folder"], ["archive_delete_2022", "folder"], ["archive_delete_2023", "folder"], ["archive_delete_2024", "folder"]]),
    ("create the directory discard_zone and erased_zone", [["discard_zone", "folder"], ["erased_zone","folder"]]),
    ("scrap file remove_this_script.py and deleting_script.py", [["remove_this_script.py", "file"],["deleting_script.py","file"]]),
    ("form directory temporary_deleted_files temporary_deleted_folders", [["temporary_deleted_files", "folder"], ["temporary_deleted_folder", "folder"]]),
    ("delete the old_backup, extra_backup folders", [["old_backup", "folder"],["extra_backup", "folder"]]),
    ("eliminate the folder deleted_files deleted_folders",[["deleted_files","folder"],["deleted_folder","folder"]]),

    #confusing with both folders and files
    ("make elimination folder and trials.txt file", [["elimination", "folder"], ["trials.txt", "file"]]),
    ("create discard_bin and a file recycle.txt", [["discard_bin", "folder"], ["recycle.txt", "file"]]),
    ("generate trash_folder and ignore_deleted.txt", [["trash_folder", "folder"], ["ignore_deleted.txt", "file"]]),
    ("create folder discardable and file throwaway.txt", [["discardable", "folder"], ["throwaway.txt", "file"]]),
    ("generate trash_folder and ignore_deleted.txt", [["trash_folder", "folder"], ["ignore_deleted.txt", "file"]]),
    ("make elimination folder and trials.txt file", [["elimination", "folder"], ["trials.txt", "file"]]),
    ("erase folders delete and eliminate and files erase.txt discard.txt remove.txt",[["delete","folder"], ["eliminate","folder"],["erase.txt","file"],["discard.txt","file"],["remove.txt","file"]]),
    ("delete the deletion, elimination folders along with remove.txt erase.txt discard.txt files",[["deletion","folder"], ["elimination","folder"],["remove.txt","file"],["erase.txt","file"],["discard.txt","file"]]),
    ("wipe out the delete_me.txt erase_this.txt failed.txt files and the ignored folder",[["delete_me.txt","file"],["erase_this.txt","file"],["failed.txt","file"],["ignored","folder"]]),
    ("remove created_today and make_tommorow folder and delete delete_this.txt file",[["created_today","folder"],["make_tommorow","folder"],["delete_this.txt","file"]]),

    #purely confusing
    ("eliminate delete", [["delete", "folder"]]),
    ("eliminate delete.txt", [["delete.txt", "file"]]),
    ("remove erase",[["erase","folder"]]),
    ("remove erase.py",[["erase.py","file"]]),
    ("wipe out wipe_out",[["wipe_out","folder"]]),
    ("wipe out wipe_out.json",[["wipe_out.json","file"]]),
    ("make create",[["create","folder"]]),
    ("add create.md",[["create.md","file"]]),
    ("form forms",[["forms","folder"]]),
    ("new form.c",[["form.c","file"]]),
